...
class GaussianPolicy(nn.Module):    # def mlp_gaussian_policy
    ...
    def forward(self, x, a=None):
        ...
        if a is not None:
            ...
            logp = None
        return pi, mu, logp_pi, logp

    def select_action(self, o):
        pi, _, _, _ = self.forward(torch.Tensor(o.reshape(1, -1)))
        return pi.cpu().detach().numpy()[0]
...
class PPOAgent():
    def __init__(self):
        ...
        self.train_v = torch.optim.Adam(self.actor_critic.policy.parameters(), lr=self.config.vf_lr)
    ...
    def update_ppo(self):
        ...
        for i in range(self.config.train_pi_iters):
            _, _, _, logp_a = self.actor_critic.policy(obs, act)
            # pi, mu, logp_pi, logp
    ...
    def main(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            for t in range(self.config.steps_per_epoch):
                a, logp_pi, _, v_t, _ = self.actor_critic(
                    torch.Tensor(o.reshape(1, -1)))  # pi, logp_pi, logp, v, mu
                ...
                self.buf.store(o, a.detach().numpy(), r, v_t.item(), logp_pi.detach().numpy())
            ...
            # save model
            if epoch % 10 == 0:
                torch.save(self.actor_critic.state_dict(), 'model_weights')
                print("Weight saved")

                self.actor_critic.load_state_dict(torch.load('model_weights'))
        ...
        while not (d or (ep_len == self.config.max_ep_len)):
            a = agent.actor_critic.policy.select_action(np.array(o))
