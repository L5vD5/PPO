...
class GaussianPolicy(nn.Module):
    ...
    def forward(self, x, a=None):
        ...
        return pi, mu, logp_pi, logp_a, pi_old
    ...
    def _apply_squashing_func(self, mu, pi, logp_pi, a, logp_a):
        ...
        logp_pi -= torch.sum(
            torch.log(self._clip_but_pass_gradient(1 - pi ** 2, l=0, u=1) + 1e-6),
            dim=1)

        if a is not None:
            logp_a -= torch.sum(
                torch.log(self._clip_but_pass_gradient(1 - a ** 2, l=0, u=1) + 1e-6),
                dim=1)

        return mu, pi, logp_pi, logp_a

    def select_action(self,o,_eval=False):
        pi,mu, _, _, _ = self.forward(torch.Tensor(o.reshape(1, -1)))
